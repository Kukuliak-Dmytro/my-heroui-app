...
export const DetailedRecipeCard = ({ id }: { id: string }) => {
    ...
    const { data: recipe, isLoading, error, isFetching } = useQuery(recipeQueryOptions(id));
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {Array.from({ length: 4 }).map((_, index) => (
                            <div key={index} className="text-center p-4 bg-default-50 rounded-lg">
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            >
                        ...
                        <CardBody className="p-6">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-6 text-sm text-foreground-600">
                                        <div className="flex items-center gap-1">
                                            <Icon icon="material-symbols:star" className="w-5 h-5 text-warning-500" />
                                            <span className="font-medium">
                                                {recipe.rating.toFixed(1)} ({recipe.reviewCount} reviews)
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <Icon icon="material-symbols:local-fire-department" className="w-5 h-5 text-danger-500" />
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                                <div className="text-center p-4 bg-default-50 rounded-lg">
                                    <Icon icon="material-symbols:schedule" className="w-6 h-6 mx-auto mb-2 text-primary" />
                                ...
                                <div className="text-center p-4 bg-default-50 rounded-lg">
                                    <Icon icon="material-symbols:group" className="w-6 h-6 mx-auto mb-2 text-primary" />
                                ...
                                <div className="text-center p-4 bg-default-50 rounded-lg">
                                    ...
                                    <div className="font-semibold">{recipe.prepTimeMinutes + recipe.cookTimeMinutes} min</div>
                                </div>
                            </div>
                ...
                <Tab key="ingredients" title={`Ingredients (${recipe.ingredients.length})`}>
                    <Card className="mt-6">
                        <CardBody className="p-6">
                            ...
                            <div className="space-y-3">
                                {recipe.ingredients.map((ingredient, index) => (
                                    <div key={index} className="flex items-center gap-3 p-3 bg-default-50 rounded-lg">
                                        <Icon icon="material-symbols:check-circle" className="w-5 h-5 text-success-500 flex-shrink-0" />
                ...
                <Tab key="instructions" title={`Instructions (${recipe.instructions.length} steps)`}>
                    <Card className="mt-6">
                        <CardBody className="p-6">
